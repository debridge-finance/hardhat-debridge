...
import { CrossChainCounter, CrossChainCounter__factory, CrossChainIncrementor, CrossChainIncrementor__factory } from "./fixture-projects/hardhat-debridge-test-env/typechain/index";
...
export async function deployContracts(hre: HardhatRuntimeEnvironment): Promise<TestSuiteState> {
  const gate = await hre.deBridge.emulator.deployGate();

  const Counter = await new CrossChainCounter__factory().connect((await hre.ethers.getSigners())[0])
  const counter = await Counter.deploy(gate.address)

  const Incrementor = await new CrossChainIncrementor__factory().connect((await hre.ethers.getSigners())[0])
